...
def get_track_by_raw_display_id_from_log(raw_display_id, track_log_file=TRACK_LOG_FILE):
    tracks = get_all_tracks_from_log(track_log_file)
    wanted = str(raw_display_id).strip()
    for track in tracks:
        raw_text = str(track.get('raw_display_id') or '').strip()
        if not raw_text:
            continue
        raw_parts = [part.strip() for part in raw_text.split(',') if part.strip()]
        if wanted in raw_parts:
            return track
    return None
