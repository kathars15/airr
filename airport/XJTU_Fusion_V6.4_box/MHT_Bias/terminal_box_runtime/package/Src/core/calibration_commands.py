...
def recalculate_calibration():
    """Recalculate calibration from saved usable history."""
    return calibrator.recalculate_from_history()
...
def show_calibration_result():
    ...
    preferred = None
    usable = [item for item in segmented if item.get('usable', False)]
    if usable:
        preferred = min(
            usable,
    ...
    if result['sample_count'] > 0:
        ...
        if segmented:
            ...
            if preferred:
                safe_print(f"  preferred segment: {preferred}")
            for item in segmented:
                safe_print(
                    ...
                    f"usable={item.get('usable', False)}, type={item.get('segment_type', 'normal')}"
                )
        safe_print("=" * 50)
    else:
