...
from core.app_config import (
    CALIBRATION_FILE, DEBUG_POINT_MHT, FRAME_HEAD_END, FRAME_HEAD_POINT,
    FRAME_HEAD_STATUS, FRAME_HEAD_TRACK, HOST_IP, HOST_PORT, POINT_RECORDS_FILE,
    RADAR_PARSE_LOG_FILE, RAW_TRACKS_FILE,
)
...
def _append_raw_tracks(timestamp, tracks, recording_enabled=True):
...
def _append_point_records(timestamp, points, ins_info=None, recording_enabled=True):
    """Persist parsed radar POINT records for offline MHT replay and diagnostics."""
    if not recording_enabled:
        return
    if not points:
        return

    ins_info = ins_info or {}
    try:
        write_header = not os.path.exists(POINT_RECORDS_FILE)
        with open(POINT_RECORDS_FILE, 'a', newline='') as f:
            writer = csv.writer(f)
            if write_header:
                writer.writerow([
                    'timestamp',
            ...
            for point in points:
                radar_heading = float(ins_info.get('radar_heading', 0.0) or 0.0)
                azimuth_absolute = float(point.get('azimuth', 0.0) or 0.0)
                azimuth_relative = float(point.get('azimuth_relative', azimuth_absolute - radar_heading) or 0.0)
                pitch_deg = float(point.get('pitch', 0.0) or 0.0)
                pitch_enu = float(point.get('pitch_enu', -pitch_deg) or 0.0)
                writer.writerow([
                    timestamp,
                    point.get('target_id'),
                    point.get('range'),
                    ...
                    pitch_enu,
                    point.get('speed', 0.0),
                    point.get('doppler', 0.0),
                    point.get('target_type', ''),
                    point.get('is_true_point', ''),
                    radar_heading,
                    ins_info.get('frame_cnt', ''),
                ])
    except Exception as e:
        safe_print(f"[point_records] write failed: {e}")
...
def _append_parse_log(line):
    try:
        with open(RADAR_PARSE_LOG_FILE, 'a', encoding='utf-8') as f:
            f.write(line + "\n")
    except Exception as e:
        safe_print(f"[parse_log] write failed: {e}")


def _log_point_parse_summary(points, ins_info):
    if not DEBUG_POINT_MHT or not points:
        return
    ...
    _append_parse_log(
        "[POINT_PARSE] "
        f"id={point.get('target_id')} "
        f"range={float(point.get('range', 0.0) or 0.0):.1f}m "
        f"abs_az={_fmt_deg(point.get('azimuth'))}deg "
        f"rel_az={_fmt_deg(point.get('azimuth_relative'))}deg "
        f"heading={_fmt_deg(radar_heading)}deg "
        f"pitch={_fmt_deg(point.get('pitch'))}deg "
        f"pitch_enu={_fmt_deg(point.get('pitch_enu'))}deg "
        f"true={point.get('is_true_point', '')}"
    )


def _log_track_parse_summary(tracks, ins_info):
    if not DEBUG_POINT_MHT or not tracks:
        return

    track = tracks[0]
    radar_heading = float(ins_info.get('radar_heading', 0.0) or 0.0)
    rel_azimuth = (float(track.get('azimuth', 0.0) or 0.0) - radar_heading) % 360.0
    _append_parse_log(
        "[TRACK_PARSE] "
        f"id={track.get('display_id')} abs_id={track.get('absolute_id')} "
        f"range={float(track.get('range', 0.0) or 0.0):.1f}m "
        f"abs_az={_fmt_deg(track.get('azimuth'))}deg "
        f"rel_az={_fmt_deg(rel_azimuth)}deg "
        f"heading={_fmt_deg(radar_heading)}deg "
        f"pitch={_fmt_deg(track.get('pitch'))}deg "
        f"mode={track.get('track_mode')} tas={track.get('is_tas')} "
        f"height={track.get('height', '')}"
    )


def receive_radar_data(Data2Process_Buffer, PointDebug_Buffer=None, recording_state=None):
    """UDP接收雷达数据，解析并存入缓冲区"""
    ...
    Save_Initial = False
    Wait_Timestamps = {}
    # POINT packets need faster release into MHT than TRACK packets did.
    Min_Num_In_Buffer = 1
    ...
    while Keep_Run:
        ...
        if frame_head == FRAME_HEAD_POINT:
            ...
            if points and len(points) > 0:
                import csv
                _append_point_records(time.time(), points, ins_info, recording_state is None or recording_state.get('enabled', False))
                _log_point_parse_summary(points, ins_info)
            ...
            # POINT packets are kept for diagnostics/calibration records only.
            # Formal tracking uses TRACK packets again.
            if PointDebug_Buffer is not None:
                measurements = []
                timestamps = []
                extra_infos = []
                ...
                if measurements:
                    PointDebug_Buffer.put({
                        'timestamp': np.array(timestamps),
                        'meas': np.array(measurements).T,
                        'sensor_name': 'Radar',
                        'infos': extra_infos,
                        'global_max_timestamp': max(timestamps) if timestamps else 0.0,
                    })
        ...
        # 处理航迹数据包
            ...
            _append_raw_tracks(time.time(), tracks, recording_state is None or recording_state.get('enabled', False))
            _log_track_parse_summary(tracks, ins_info)
            ...
            for track in tracks:
                ...
                extra_info = {
                    ...
                    'speed': track['speed'],
                    'track_mode_bits': track.get('track_mode_bits'),
                    'track_mode': track.get('track_mode'),
                    'is_tas': track['is_tas'],
