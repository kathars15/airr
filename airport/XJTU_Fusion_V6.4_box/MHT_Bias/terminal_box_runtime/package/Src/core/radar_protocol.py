...
def parse_radar_point_packet(data):
    ...
    if point_num > 0:
        ...
        for i in range(point_num):
            ...
            if pitch > 32767:
                pitch = pitch - 65536
            # POINT packet pitch sign is opposite to TRACK packet convention.
            # Normalize here so both parsed outputs use positive-up pitch.
...
def parse_radar_track_packet(data):
    ...
    if track_num > 0:
        ...
        for i in range(track_num):
            ...
            track_mode_bits = track_flag & 0x3
            is_tas = track_mode_bits == 1
            if track_mode_bits == 1:
                track_mode = 'TAS'
            elif track_mode_bits == 0:
                track_mode = 'TWS'
            else:
                track_mode = 'UNKNOWN'
            ...
            if range_val > 0 and range_val < 100000:
                track = {
                    ...
                    'target_type': target_type,
                    'speed': speed_ms,
                    'track_mode_bits': track_mode_bits,
