# calibration.py
"""
雷达与光电校准模块。

在线校准仍保留原来的采集、配对、保存源数据能力；发送光电时优先使用
6DoF（位置+旋转）参数，缺少 6DoF 参数时回退到位置偏移/角度偏移。
"""
from core.app_config import FAKE_DIS
import json
import os
import time
from collections import deque

import numpy as np
from core.track_smoothing import OpticalAngleSmoother, TrackAngleSmoother

try:
    from core.app_config import (
        CALIBRATION_MAX_GEOMETRY_COND,
        CALIBRATION_MAX_MAX_ERROR_DEG,
        CALIBRATION_MAX_MEAN_AZ_ERROR_DEG,
        CALIBRATION_MAX_MEAN_ERROR_DEG,
        CALIBRATION_MAX_MEAN_PITCH_ERROR_DEG,
        CALIBRATION_MAX_OFFSET_X_M,
        CALIBRATION_MAX_OFFSET_Y_M,
        CALIBRATION_MAX_OFFSET_Z_M,
        CALIBRATION_MIN_RANGE,
        CALIBRATION_PAIR_TIME_WINDOW,
        SCRIPT_DIR,
    )
except ImportError:
    CALIBRATION_MIN_RANGE = 700.0
    CALIBRATION_PAIR_TIME_WINDOW = 0.15
    SCRIPT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    CALIBRATION_MAX_OFFSET_X_M = 20.0
    CALIBRATION_MAX_OFFSET_Y_M = 20.0
    CALIBRATION_MAX_OFFSET_Z_M = 10.0
    CALIBRATION_MAX_MEAN_ERROR_DEG = 0.30
    CALIBRATION_MAX_MAX_ERROR_DEG = 0.80
    CALIBRATION_MAX_MEAN_AZ_ERROR_DEG = 0.25
    CALIBRATION_MAX_MEAN_PITCH_ERROR_DEG = 0.25
    CALIBRATION_MAX_GEOMETRY_COND = 1.0e5

DEFAULT_CALIBRATION_DATA_DIR = os.path.join(SCRIPT_DIR, "calibration_data")
CALIBRATION_HISTORY_FILE = "calibration_history.json"
CALIBRATION_SESSION_DIR = "cal_sessions"
CALIBRATION_RAW_RECORD_DIR = "raw_pair_records"
CALIBRATION_READABLE_LOG_DIR = "readable_pair_logs"
STABILITY_BIN_SIZE_M = 100.0
STABILITY_MIN_SAMPLES_PER_BIN = 3
STABILITY_MAX_PITCH_STD_DEG = 0.35
STABILITY_MAX_BIN_PITCH_JUMP_DEG = 1.0
STABILITY_MAX_SAMPLE_GAP_M = 150.0
SEGMENT_USABLE_MEAN_ERROR_DEG = 2
SEGMENT_USABLE_MAX_ERROR_DEG = 5
SEGMENT_USABLE_MIN_SAMPLES = 4
LOW_PITCH_STD_DEG = 0.2
LOW_PITCH_SPAN_DEG = 0.5
CAL_RADAR_SMOOTH_WINDOW = 3
CAL_RADAR_SMOOTH_MAX_AGE_SEC = 5.0
CAL_OPTICAL_SMOOTH_WINDOW = 3
CAL_OPTICAL_SMOOTH_MAX_AGE_SEC = 5.0


def _empty_6dof_params(method):
    return {
        'dx': 0.0,
        'dy': 0.0,
        'dz': 0.0,
        'az_rotation': 0.0,
        'pitch_rotation': 0.0,
        'mean_error_deg': 0.0,
        'std_error_deg': 0.0,
        'max_error_deg': 0.0,
        'timestamp': 0,
        'sample_count': 0,
        'method': method,
        'geometry_condition': 0.0,
    }


def _empty_segmented_6dof_entry():
    data = _empty_6dof_params('segmented_6dof')
    data.update({
        'range_min_m': 0.0,
        'range_max_m': 0.0,
        'range_label': '',
        'usable': False,
        'usable_reason': '',
        'segment_type': 'normal',
        'segment_solver': '',
        'vertical_weak': False,
        'mean_az_error_deg': 0.0,
        'max_az_error_deg': 0.0,
        'mean_pitch_error_deg': 0.0,
        'max_pitch_error_deg': 0.0,
    })
    return data


class RadarOpticalCalibrator:
    def __init__(self, data_dir=DEFAULT_CALIBRATION_DATA_DIR):
        self.data_dir = data_dir
        os.makedirs(data_dir, exist_ok=True)

        self.radar_measurements = []
        self.optical_measurements = []
        self.current_target_id = None
        self.calibration_mode = False
        self.calibration_target_history = []

        self.radar_buffer = deque(maxlen=10)
        self.optical_buffer = deque(maxlen=10)
        self.radar_stability_samples = []
        self.radar_stability_report = self._empty_stability_report()
        self.radar_smoother = TrackAngleSmoother(
            window_size=CAL_RADAR_SMOOTH_WINDOW,
            max_age_sec=CAL_RADAR_SMOOTH_MAX_AGE_SEC,
        )
        self.optical_smoother = OpticalAngleSmoother(
            window_size=CAL_OPTICAL_SMOOTH_WINDOW,
            max_age_sec=CAL_OPTICAL_SMOOTH_MAX_AGE_SEC,
        )

        self.calibration_result = {
            'azimuth_offset': 0.0,
            'pitch_offset': 0.0,
            'azimuth_scale': 1.0,
            'pitch_scale': 1.0,
            'timestamp': 0,
            'sample_count': 0,
        }

        self.position_offset = {
            'dx': 0.0,
            'dy': 0.0,
            'dz': 0.0,
            'timestamp': 0,
            'sample_count': 0,
            'use_position': False,
            'method': 'position_bias_bearing_task',
            'mean_error_deg': 0.0,
            'std_error_deg': 0.0,
            'max_error_deg': 0.0,
            'geometry_condition': 0.0,
            'position_bias_type': 'task_driven_equivalent_offset',
        }

        self.RANGE_THRESHOLD = 1200.0
        self.far_params = _empty_6dof_params('far_6dof')
        self.near_params = _empty_6dof_params('near_6dof')
        self.segmented_6dof_params = []

        self.load_calibration()
        self.load_position_offset()
        self.load_6dof_params()

    def _get_session_dir(self):
        session_dir = os.path.join(self.data_dir, CALIBRATION_SESSION_DIR)
        os.makedirs(session_dir, exist_ok=True)
        return session_dir

    def _get_raw_record_dir(self):
        path = os.path.join(self.data_dir, CALIBRATION_RAW_RECORD_DIR)
        os.makedirs(path, exist_ok=True)
        return path

    def _get_readable_log_dir(self):
        path = os.path.join(self.data_dir, CALIBRATION_READABLE_LOG_DIR)
        os.makedirs(path, exist_ok=True)
        return path

    def _history_file_path(self):
        return os.path.join(self.data_dir, CALIBRATION_HISTORY_FILE)

    def _load_history_batches(self):
        file_path = self._history_file_path()
        if not os.path.exists(file_path):
            return []
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            return data.get('batches', []) if isinstance(data, dict) else []
        except Exception:
            return []

    def _save_history_batches(self, batches):
        file_path = self._history_file_path()
        data = {
            'timestamp': time.time(),
            'batch_count': len(batches),
            'batches': batches,
        }
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

    def _is_low_pitch_segment(self, stable_range):
        pitch_span = float(stable_range.get('pitch_max_deg', 0.0) - stable_range.get('pitch_min_deg', 0.0))
        pitch_std = float(stable_range.get('pitch_std_max_deg', 0.0))
        return pitch_std <= LOW_PITCH_STD_DEG and pitch_span <= LOW_PITCH_SPAN_DEG

    def _compute_component_errors(self, offset, measurements):
        if not measurements:
            return {
                'mean_az_error_deg': float('inf'),
                'max_az_error_deg': float('inf'),
                'mean_pitch_error_deg': float('inf'),
                'max_pitch_error_deg': float('inf'),
            }

        az_errors = []
        pitch_errors = []
        for item in measurements:
            az_rad = np.radians(item['radar_az'])
            pitch_rad = np.radians(item['radar_pitch'])
            x = item['radar_range'] * np.cos(pitch_rad) * np.sin(az_rad)
            y = item['radar_range'] * np.cos(pitch_rad) * np.cos(az_rad)
            z = item['radar_range'] * np.sin(pitch_rad)
            rel_x = x - offset[0]
            rel_y = y - offset[1]
            rel_z = z - offset[2]
            opt_range = np.sqrt(rel_x**2 + rel_y**2 + rel_z**2)
            pred_az = np.degrees(np.arctan2(rel_x, rel_y)) % 360.0
            pred_pitch = np.degrees(np.arcsin(rel_z / opt_range)) if opt_range > 0 else 0.0
            az_error = abs((pred_az - item['optical_az'] + 180.0) % 360.0 - 180.0)
            pitch_error = abs(pred_pitch - item['optical_pitch'])
            az_errors.append(float(az_error))
            pitch_errors.append(float(pitch_error))

        return {
            'mean_az_error_deg': float(np.mean(az_errors)),
            'max_az_error_deg': float(np.max(az_errors)),
            'mean_pitch_error_deg': float(np.mean(pitch_errors)),
            'max_pitch_error_deg': float(np.max(pitch_errors)),
        }

    def _evaluate_segment_quality(self, segment_params):
        sample_count = int(segment_params.get('sample_count', 0))
        mean_error = float(segment_params.get('mean_error_deg', 0.0))
        max_error = float(segment_params.get('max_error_deg', 0.0))
        segment_type = segment_params.get('segment_type', 'normal')
        vertical_weak = bool(segment_params.get('vertical_weak', False))
        mean_az_error = float(segment_params.get('mean_az_error_deg', float('inf')))
        max_az_error = float(segment_params.get('max_az_error_deg', float('inf')))

        if sample_count < SEGMENT_USABLE_MIN_SAMPLES:
            return False, 'sample_count_too_small'

        if segment_type == 'low_pitch_variation':
            if mean_az_error <= SEGMENT_USABLE_MEAN_ERROR_DEG and max_az_error <= SEGMENT_USABLE_MAX_ERROR_DEG:
                if vertical_weak:
                    return True, 'low_pitch_variation_horizontal_only'
                return True, 'low_pitch_variation'
            return False, 'low_pitch_variation_error_too_large'

        if mean_error <= SEGMENT_USABLE_MEAN_ERROR_DEG and max_error <= SEGMENT_USABLE_MAX_ERROR_DEG:
            return True, 'normal_segment'
        return False, 'error_too_large'

    def _calculate_low_pitch_segment_params(self, measurements, range_min_m, range_max_m):
        try:
            from scipy.optimize import least_squares
            from tools.cal_offset import summarize_angle_offsets
        except ImportError as exc:
            return {'success': False, 'reason': f'cannot import dependency: {exc}'}

        if len(measurements) < STABILITY_MIN_SAMPLES_PER_BIN:
            return {'success': False, 'reason': 'not enough samples for low-pitch solver'}

        def residuals(params):
            dx, dy = params
            values = []
            for item in measurements:
                az_rad = np.radians(item['radar_az'])
                pitch_rad = np.radians(item['radar_pitch'])
                x = item['radar_range'] * np.cos(pitch_rad) * np.sin(az_rad)
                y = item['radar_range'] * np.cos(pitch_rad) * np.cos(az_rad)
                rel_x = x - dx
                rel_y = y - dy
                pred_az = np.degrees(np.arctan2(rel_x, rel_y)) % 360.0
                az_error = (pred_az - item['optical_az'] + 180.0) % 360.0 - 180.0
                values.append(float(az_error))
            return np.array(values, dtype=float)

        x0 = np.array([0.0, 0.0], dtype=float)
        result = least_squares(residuals, x0, bounds=([-3000.0, -3000.0], [3000.0, 3000.0]), method='trf')
        if not result.success:
            return {'success': False, 'reason': f'low_pitch_solver failed: {result.message}'}

        dx, dy = [float(x) for x in result.x]
        radar_pitches = np.array([item['radar_pitch'] for item in measurements], dtype=float)
        optical_pitches = np.array([item['optical_pitch'] for item in measurements], dtype=float)
        dz = 0.0
        offset = np.array([dx, dy, dz], dtype=float)
        component_errors = self._compute_component_errors(offset, measurements)
        total_errors = np.sqrt(
            np.square(component_errors['mean_az_error_deg']) + np.square(component_errors['mean_pitch_error_deg'])
        )
        validation = {
            'mean_error_deg': float(total_errors),
            'std_error_deg': 0.0,
            'max_error_deg': float(max(component_errors['max_az_error_deg'], component_errors['max_pitch_error_deg'])),
        }
        angle_stats = summarize_angle_offsets(measurements)
        return {
            'success': True,
            'offset': offset,
            'sample_count': len(measurements),
            'validation': validation,
            'angle_stats': angle_stats,
            'method': 'low_pitch_horizontal_solver',
            'component_errors': component_errors,
            'segment_type': 'low_pitch_variation',
            'vertical_weak': True,
            'segment_solver': 'low_pitch_solver',
            'range_min_m': range_min_m,
            'range_max_m': range_max_m,
            'pitch_reference_deg': float(np.median(radar_pitches)),
            'optical_pitch_reference_deg': float(np.median(optical_pitches)),
        }

    def _build_history_batch(self):
        return {
            'timestamp': time.time(),
            'target_id': self.current_target_id,
            'target_history': list(self.calibration_target_history),
            'sample_count': len(self.radar_measurements),
            'calibration_result': self._with_source_measurements(self.calibration_result),
            'position_offset': self._with_source_measurements(self.position_offset),
            'segments': [dict(item) for item in self.segmented_6dof_params],
            'stability_report': self.radar_stability_report,
        }

    def _sanitize_target_for_filename(self, target_id):
        text = str(target_id or "unknown").strip()
        return "".join(ch if ch.isalnum() or ch in ('-', '_') else '_' for ch in text) or "unknown"

    def _build_cal_session_snapshot(self):
        source_measurements = self.get_source_measurements()
        return {
            'schema_version': 1,
            'saved_at': time.time(),
            'target_id': self.current_target_id,
            'target_history': list(self.calibration_target_history),
            'sample_count': source_measurements.get('sample_count', 0),
            'min_radar_range_m': float(CALIBRATION_MIN_RANGE),
            'pair_time_window_sec': float(CALIBRATION_PAIR_TIME_WINDOW),
            'source_measurements': source_measurements,
            'radar_stability_samples': list(self.radar_stability_samples),
            'stability_report': self.radar_stability_report,
            'result_summary': {
                'calibration_result': self._with_source_measurements(self.calibration_result),
                'position_offset': self._with_source_measurements(self.position_offset),
                'segments': [dict(item) for item in self.segmented_6dof_params],
                'stability_report': self.radar_stability_report,
            },
        }

    def save_calibration_session_snapshot(self):
        snapshot = self._build_cal_session_snapshot()
        if snapshot.get('sample_count', 0) <= 0:
            return None

        timestamp_text = time.strftime('%Y%m%d_%H%M%S', time.localtime(snapshot['saved_at']))
        target_text = self._sanitize_target_for_filename(snapshot.get('target_id'))
        file_path = os.path.join(self._get_session_dir(), f"cal_session_{timestamp_text}_{target_text}.json")
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(snapshot, f, indent=2, ensure_ascii=False)
        print(f"[校准] 会话快照已保存: {file_path}")
        return file_path

    def save_pair_record_files(self):
        source = self.get_source_measurements()
        if source.get('sample_count', 0) <= 0:
            return None, None

        timestamp_text = time.strftime('%Y%m%d_%H%M%S', time.localtime(time.time()))
        target_text = self._sanitize_target_for_filename(source.get('target_id'))

        raw_path = os.path.join(
            self._get_raw_record_dir(),
            f"raw_pairs_{timestamp_text}_{target_text}.json",
        )
        with open(raw_path, 'w', encoding='utf-8') as f:
            json.dump(source, f, indent=2, ensure_ascii=False)

        readable_path = os.path.join(
            self._get_readable_log_dir(),
            f"readable_pairs_{timestamp_text}_{target_text}.txt",
        )
        with open(readable_path, 'w', encoding='utf-8-sig') as f:
            for item in source.get('samples', []):
                radar = item.get('radar', {})
                optical = item.get('optical', {})
                dt = float(item.get('time_diff_sec') or 0.0)
                f.write(
                    f"[校准] 配对成功: 雷达({float(radar.get('azimuth', 0.0)):.1f}°, "
                    f"{float(radar.get('pitch', 0.0)):.1f}°, {float(radar.get('range', 0.0)):.0f}m) -> "
                    f"光电({float(optical.get('azimuth', 0.0)):.1f}°, {float(optical.get('pitch', 0.0)):.1f}°, "
                    f"{float(optical.get('range', 0.0)):.0f}m), 时间差={dt:.2f}s\n"
                )

        print(f"[calibration] raw pair records saved: {raw_path}")
        print(f"[calibration] readable pair log saved: {readable_path}")
        return raw_path, readable_path

    def save_calibration_batch(self):
        batch = self._build_history_batch()
        if not batch.get('segments'):
            return
        batches = self._load_history_batches()
        batches.append(batch)
        self._save_history_batches(batches)

    def _collect_historical_usable_samples(self):
        batches = self._load_history_batches()
        merged_samples = []
        for batch in batches:
            for segment in batch.get('segments', []):
                if not segment.get('usable', False):
                    continue
                source = segment.get('source_measurements', {})
                merged_samples.extend(source.get('samples', []))
        return merged_samples

    def recalculate_from_history(self):
        merged_samples = self._collect_historical_usable_samples()
        if len(merged_samples) < 5:
            print(f"[calibration] not enough historical usable samples, current={len(merged_samples)}")
            return False

        self.load_source_measurements(
            {
                'target_id': 'history-merged',
                'samples': merged_samples,
            },
            target_history=['history-merged'],
        )
        return self.finalize_calibration_from_loaded_samples()

    def load_source_measurements(self, source_measurements, target_history=None, stability_samples=None):
        self.radar_measurements = []
        self.optical_measurements = []
        self.radar_stability_samples = []
        self.radar_buffer.clear()
        self.optical_buffer.clear()
        self.radar_smoother.reset()
        self.optical_smoother.reset()

        self.current_target_id = source_measurements.get('target_id') or 'offline-session'
        self.calibration_target_history = list(target_history or [self.current_target_id])

        for item in source_measurements.get('samples', []):
            radar = item.get('radar', {})
            optical = item.get('optical', {})
            self.radar_measurements.append({
                'timestamp': radar.get('timestamp'),
                'track_id': radar.get('track_id'),
                'is_tas': radar.get('is_tas'),
                'track_mode': radar.get('track_mode'),
                'azimuth': radar.get('azimuth'),
                'pitch': radar.get('pitch'),
                'range': radar.get('range'),
                'raw_azimuth': radar.get('raw_azimuth', radar.get('azimuth')),
                'raw_pitch': radar.get('raw_pitch', radar.get('pitch')),
                'raw_range': radar.get('raw_range', radar.get('range')),
            })
            self.optical_measurements.append({
                'timestamp': optical.get('timestamp'),
                'azimuth': optical.get('azimuth'),
                'pitch': optical.get('pitch'),
                'range': optical.get('range'),
                'status': optical.get('status'),
                'raw_azimuth': optical.get('raw_azimuth', optical.get('azimuth')),
                'raw_pitch': optical.get('raw_pitch', optical.get('pitch')),
                'raw_range': optical.get('raw_range', optical.get('range')),
            })

        if stability_samples is not None:
            self.radar_stability_samples = list(stability_samples)
        else:
            for item in source_measurements.get('samples', []):
                radar = item.get('radar', {})
                self.add_radar_stability_sample(
                    radar.get('track_id'),
                    radar.get('raw_azimuth', radar.get('azimuth')),
                    radar.get('raw_pitch', radar.get('pitch')),
                    radar.get('raw_range', radar.get('range')),
                    radar.get('timestamp'),
                )

    def finalize_calibration_from_loaded_samples(self, save_outputs=True):
        if len(self.radar_measurements) < 5:
            print(f"[校准] 数据不足，需要至少5组数据，当前有{len(self.radar_measurements)}组")
            self.print_radar_stability_summary()
            if save_outputs:
                self.save_radar_stability_report()
            return False

        self.analyze_radar_stability()
        self.print_radar_stability_summary()
        if save_outputs:
            self.save_radar_stability_report()

        print("[校准] 使用位置偏移优先算法计算参数...")
        if self.calculate_position_offset():
            self.calculate_segmented_6dof_params_v2()
            if save_outputs:
                self.save_calibration()
                self.save_position_offset()
                self.save_6dof_params()
            print("[校准] 位置偏移优先校准完成！")
            print(
                f"[校准] 等效位置偏移: 东偏移 {self.position_offset['dx']:.2f}m, "
                f"北偏移 {self.position_offset['dy']:.2f}m, 高偏移 {self.position_offset['dz']:.2f}m"
            )
            print(f"[校准] 样本数量: {self.position_offset['sample_count']}")
            mean_error = self.position_offset.get('mean_error_deg')
            max_error = self.position_offset.get('max_error_deg')
            if mean_error is not None and max_error is not None:
                print(f"[校准] 重投影角度误差: 均值 {mean_error:.3f}°, 最大 {max_error:.3f}°")
            return True

        print("[校准] 位置偏移求解失败，回退到角度偏移校准...")
        self._calculate_calibration()
        self.calculate_segmented_6dof_params_v2()
        if save_outputs:
            self.save_calibration()
            self.save_6dof_params()
        print("[校准] 角度偏移校准完成！")
        print(f"[校准] 方位角偏移: {self.calibration_result['azimuth_offset']:.2f}°")
        print(f"[校准] 俯仰角偏移: {self.calibration_result['pitch_offset']:.2f}°")
        print(f"[校准] 样本数量: {self.calibration_result['sample_count']}")
        return True

    def start_calibration(self, target_id):
        """开始校准。"""
        self.calibration_mode = True
        self.current_target_id = target_id
        self.calibration_target_history = [target_id]
        self.radar_measurements = []
        self.optical_measurements = []
        self.radar_stability_samples = []
        self.radar_stability_report = self._empty_stability_report()
        self.radar_buffer.clear()
        self.optical_buffer.clear()
        self.radar_smoother.reset()
        self.optical_smoother.reset()
        print(f"[校准] 开始校准，目标: {target_id}")
        print(f"[校准] 雷达扫描周期: 4秒，建议采集10-20组数据")
        print(f"[校准] 距离过滤: 仅采集雷达距离 >= {CALIBRATION_MIN_RANGE:.0f}m 的配对数据")
        return True

    def switch_calibration_target(self, target_id):
        """Switch calibration target without clearing collected samples."""
        if not self.calibration_mode:
            return self.start_calibration(target_id)

        if target_id == self.current_target_id:
            print(
                f"[calibration] target unchanged: {target_id}, "
                f"keep {len(self.radar_measurements)} paired samples"
            )
            return 'unchanged'

        self.current_target_id = target_id
        if target_id not in self.calibration_target_history:
            self.calibration_target_history.append(target_id)
        self.radar_buffer.clear()
        self.optical_buffer.clear()
        print(
            f"[calibration] switched target: {target_id}, "
            f"keep {len(self.radar_measurements)} paired samples and continue"
        )
        return 'switched'

    def stop_calibration(self):
        """停止校准并计算/保存参数。"""
        self.calibration_mode = False
        ok = self.finalize_calibration_from_loaded_samples()
        if ok:
            self.save_calibration_batch()
            self.save_calibration_session_snapshot()
            self.save_pair_record_files()
        return ok

    def add_radar_measurement(self, track_id, azimuth, pitch, range_m, timestamp=None, is_tas=None, track_mode=None, device_timestamp=None):
        """添加雷达测量数据。"""
        if not self.calibration_mode or track_id != self.current_target_id:
            return

        if timestamp is None:
            timestamp = time.time()

        raw_range = range_m
        raw_azimuth = azimuth
        raw_pitch = pitch
        range_m, azimuth, pitch = self.radar_smoother.smooth(
            track_id,
            timestamp,
            range_m,
            azimuth,
            pitch,
        )

        self.add_radar_stability_sample(track_id, raw_azimuth, raw_pitch, raw_range, timestamp)
        self.radar_buffer.append({
            'timestamp': timestamp,
            'device_timestamp': device_timestamp,
            'track_id': track_id,
            'is_tas': is_tas,
            'track_mode': track_mode,
            'azimuth': azimuth,
            'pitch': pitch,
            'range': range_m,
            'raw_azimuth': raw_azimuth,
            'raw_pitch': raw_pitch,
            'raw_range': raw_range,
        })
        self._try_pair_measurement()

    def _empty_stability_report(self):
        return {
            'target_id': self.current_target_id,
            'target_history': list(self.calibration_target_history),
            'sample_count': 0,
            'bin_size_m': STABILITY_BIN_SIZE_M,
            'min_samples_per_bin': STABILITY_MIN_SAMPLES_PER_BIN,
            'max_pitch_std_deg': STABILITY_MAX_PITCH_STD_DEG,
            'max_bin_pitch_jump_deg': STABILITY_MAX_BIN_PITCH_JUMP_DEG,
            'max_sample_gap_m': STABILITY_MAX_SAMPLE_GAP_M,
            'stable_ranges': [],
            'bins': [],
            'timestamp': 0,
        }

    def add_radar_stability_sample(self, track_id, azimuth, pitch, range_m, timestamp=None):
        """记录雷达俯仰稳定性分析样本。"""
        try:
            range_value = float(range_m)
            pitch_value = float(pitch)
            azimuth_value = float(azimuth)
        except (TypeError, ValueError):
            return

        if range_value <= 0:
            return

        if timestamp is None:
            timestamp = time.time()

        self.radar_stability_samples.append({
            'timestamp': float(timestamp),
            'track_id': track_id,
            'range': range_value,
            'azimuth': azimuth_value,
            'pitch': pitch_value,
        })

    def analyze_radar_stability(self):
        """按距离分箱分析雷达俯仰角稳定范围。"""
        samples = list(self.radar_stability_samples)
        report = self._empty_stability_report()
        report['target_id'] = self.current_target_id
        report['sample_count'] = len(samples)
        report['timestamp'] = time.time()

        if len(samples) < STABILITY_MIN_SAMPLES_PER_BIN:
            self.radar_stability_report = report
            return report

        grouped = {}
        for sample in samples:
            bin_start = int(sample['range'] // STABILITY_BIN_SIZE_M) * int(STABILITY_BIN_SIZE_M)
            grouped.setdefault(bin_start, []).append(sample)

        bins = []
        for bin_start in sorted(grouped):
            values = grouped[bin_start]
            ranges = np.array([v['range'] for v in values], dtype=float)
            pitches = np.array([v['pitch'] for v in values], dtype=float)
            pitch_std = float(np.std(pitches))
            pitch_mean = float(np.mean(pitches))
            pitch_median = float(np.median(pitches))
            pitch_min = float(np.min(pitches))
            pitch_max = float(np.max(pitches))
            stable = (
                len(values) >= STABILITY_MIN_SAMPLES_PER_BIN
                and pitch_std <= STABILITY_MAX_PITCH_STD_DEG
            )
            bins.append({
                'start_m': float(bin_start),
                'end_m': float(bin_start + STABILITY_BIN_SIZE_M),
                'sample_count': len(values),
                'range_min_m': float(np.min(ranges)),
                'range_max_m': float(np.max(ranges)),
                'pitch_mean_deg': pitch_mean,
                'pitch_median_deg': pitch_median,
                'pitch_std_deg': pitch_std,
                'pitch_min_deg': pitch_min,
                'pitch_max_deg': pitch_max,
                'stable': stable,
            })

        report['bins'] = bins
        report['stable_ranges'] = self._find_stable_ranges_from_samples(samples)
        self.radar_stability_report = report
        return report

    def _find_stable_ranges_from_samples(self, samples):
        """从按距离排序的样本中寻找俯仰角稳定的连续距离段。"""
        sorted_samples = sorted(samples, key=lambda item: item['range'])
        ranges = []
        current = []

        def summarize(segment):
            if len(segment) < STABILITY_MIN_SAMPLES_PER_BIN:
                return None
            pitches = np.array([item['pitch'] for item in segment], dtype=float)
            distances = np.array([item['range'] for item in segment], dtype=float)
            pitch_std = float(np.std(pitches))
            if pitch_std > STABILITY_MAX_PITCH_STD_DEG:
                return None
            return {
                'start_m': float(np.min(distances)),
                'end_m': float(np.max(distances)),
                'range_min_m': float(np.min(distances)),
                'range_max_m': float(np.max(distances)),
                'sample_count': len(segment),
                'pitch_mean_deg': float(np.mean(pitches)),
                'pitch_median_start_deg': float(np.median(pitches)),
                'pitch_median_end_deg': float(np.median(pitches)),
                'pitch_std_max_deg': pitch_std,
                'pitch_min_deg': float(np.min(pitches)),
                'pitch_max_deg': float(np.max(pitches)),
            }

        def flush_current():
            nonlocal current
            summary = summarize(current)
            if summary:
                ranges.append(summary)
            current = []

        for sample in sorted_samples:
            if not current:
                current = [sample]
                continue

            previous = current[-1]
            gap = sample['range'] - previous['range']
            pitch_jump = abs(sample['pitch'] - previous['pitch'])
            candidate = current + [sample]
            candidate_pitches = np.array([item['pitch'] for item in candidate], dtype=float)
            candidate_std = float(np.std(candidate_pitches))

            if (
                gap <= STABILITY_MAX_SAMPLE_GAP_M
                and pitch_jump <= STABILITY_MAX_BIN_PITCH_JUMP_DEG
                and candidate_std <= STABILITY_MAX_PITCH_STD_DEG
            ):
                current = candidate
            else:
                flush_current()
                current = [sample]

        flush_current()
        return ranges

    def format_radar_stability_summary(self):
        report = self.analyze_radar_stability()
        ranges = report.get('stable_ranges', [])
        if not ranges:
            return (
                f"[雷达稳定性] 当前样本 {report.get('sample_count', 0)} 组，"
                "暂未识别到俯仰稳定距离段"
            )

        parts = []
        for item in ranges:
            parts.append(
                f"{item['range_min_m']:.0f}-{item['range_max_m']:.0f}m"
                f"(n={item['sample_count']}, std≤{item['pitch_std_max_deg']:.2f}°)"
            )
        return (
            f"[雷达稳定性] 当前样本 {report.get('sample_count', 0)} 组，"
            f"俯仰稳定距离段: {', '.join(parts)}"
        )

    def print_radar_stability_summary(self):
        print(self.format_radar_stability_summary())

    def save_radar_stability_report(self):
        file_path = os.path.join(self.data_dir, 'radar_stability_report.json')
        report = self.analyze_radar_stability()
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        print(f"[雷达稳定性] 报告已保存: {file_path}")

    def add_optical_measurement(self, azimuth, pitch, timestamp=None, optical_status=None, opt_range=None, device_timestamp=None):
        """添加光电实际角度和距离。"""
        if not self.calibration_mode:
            return

        if optical_status != 2:
            return

        if timestamp is None:
            timestamp = time.time()

        raw_azimuth = azimuth
        raw_pitch = pitch
        raw_range = opt_range if opt_range else 0
        azimuth, pitch, opt_range = self.optical_smoother.smooth(
            timestamp,
            azimuth,
            pitch,
            opt_range,
        )

        self.optical_buffer.append({
            'timestamp': timestamp,
            'device_timestamp': device_timestamp,
            'azimuth': azimuth,
            'pitch': pitch,
            'range': opt_range if opt_range else 0,
            'status': optical_status,
            'raw_azimuth': raw_azimuth,
            'raw_pitch': raw_pitch,
            'raw_range': raw_range,
        })
        self._try_pair_measurement()

    def _try_pair_measurement(self):
        """尝试配对雷达和光电测量数据。"""
        if len(self.radar_buffer) == 0 or len(self.optical_buffer) == 0:
            return

        best_pair = None
        best_diff = float('inf')

        for r in list(self.radar_buffer):
            for o in list(self.optical_buffer):
                radar_time = r.get('device_timestamp')
                optical_time = o.get('device_timestamp')
                if radar_time is None or optical_time is None:
                    radar_time = r['timestamp']
                    optical_time = o['timestamp']
                time_diff = abs(float(radar_time) - float(optical_time))
                radar_range = r.get('range', 0) or 0
                if radar_range < CALIBRATION_MIN_RANGE:
                    continue
                if time_diff <= CALIBRATION_PAIR_TIME_WINDOW and time_diff < best_diff:
                    best_pair = (r, o)
                    best_diff = time_diff

        if best_pair:
            r, o = best_pair
            self.radar_measurements.append(r)
            self.optical_measurements.append(o)
            print(
                f"[校准] 配对成功: 雷达({r['azimuth']:.1f}°, {r['pitch']:.1f}°, {r['range']:.0f}m) -> "
                f"光电({o['azimuth']:.1f}°, {o['pitch']:.1f}°, {o['range']:.0f}m), 时间差={best_diff:.2f}s"
            )
            self.radar_buffer.clear()
            self.optical_buffer.clear()

    def _calculate_calibration(self):
        """计算角度偏移校准参数。"""
        try:
            from tools.cal_offset import summarize_angle_offsets
        except ImportError:
            summarize_angle_offsets = None

        if summarize_angle_offsets:
            measurements = self.to_cal_offset_measurements()
            stats = summarize_angle_offsets(measurements)
            self.calibration_result = {
                'azimuth_offset': float(stats['azimuth_offset_median']),
                'pitch_offset': float(stats['pitch_offset_median']),
                'azimuth_scale': 1.0,
                'pitch_scale': 1.0,
                'timestamp': time.time(),
                'sample_count': len(measurements),
                'method': 'cal_offset_angle_median',
            }
            return

        n = len(self.radar_measurements)
        radar_az = np.array([m['azimuth'] for m in self.radar_measurements], dtype=float)
        radar_pitch = np.array([m['pitch'] for m in self.radar_measurements], dtype=float)
        opt_az = np.array([m['azimuth'] for m in self.optical_measurements], dtype=float)
        opt_pitch = np.array([m['pitch'] for m in self.optical_measurements], dtype=float)
        az_diff = (opt_az - radar_az + 180.0) % 360.0 - 180.0
        pitch_diff = opt_pitch - radar_pitch

        self.calibration_result = {
            'azimuth_offset': float(np.median(az_diff)),
            'pitch_offset': float(np.median(pitch_diff)),
            'azimuth_scale': 1.0,
            'pitch_scale': 1.0,
            'timestamp': time.time(),
            'sample_count': n,
            'method': 'angle_median',
        }

    def calculate_position_offset(self):
        """使用 bearing-only 任务驱动算法计算等效位置偏移。"""
        measurements = self.to_cal_offset_measurements()
        if len(measurements) < 5:
            print(f"[校准] 位置偏移求解需要至少5组有效配对数据，当前{len(measurements)}组")
            return False

        try:
            from tools.cal_offset import calculate_calibration_from_measurements
        except ImportError as exc:
            print(f"[校准] 无法导入 tools.cal_offset: {exc}")
            return False

        result = calculate_calibration_from_measurements(measurements)
        angle_stats = result.get('angle_stats', {})
        self.calibration_result = {
            'azimuth_offset': float(angle_stats.get('azimuth_offset_median', 0.0)),
            'pitch_offset': float(angle_stats.get('pitch_offset_median', 0.0)),
            'azimuth_scale': 1.0,
            'pitch_scale': 1.0,
            'timestamp': time.time(),
            'sample_count': int(result.get('sample_count', len(measurements))),
            'method': 'cal_offset_angle_median',
        }

        if not result.get('success'):
            print(f"[校准] {result.get('reason', '位置偏移计算失败')}")
            return False

        offset = result['offset']
        validation = result.get('validation', {})
        geometry_condition = float(result.get('geometry_condition', 0.0) or 0.0)
        dx, dy, dz = offset

        print("[校准] 位置偏移计算:")
        print(f"       等效位置偏移: 东偏移 {dx:.2f}m, 北偏移 {dy:.2f}m, 高偏移 {dz:.2f}m")
        print(
            "       重投影角度误差: "
            f"均值 {validation.get('mean_error_deg', 0.0):.3f}°, "
            f"标准差 {validation.get('std_error_deg', 0.0):.3f}°, "
            f"最大 {validation.get('max_error_deg', 0.0):.3f}°"
        )
        print(f"       几何条件数: {geometry_condition:.3f}")

        self.position_offset = {
            'dx': float(dx),
            'dy': float(dy),
            'dz': float(dz),
            'timestamp': time.time(),
            'sample_count': int(result.get('sample_count', len(measurements))),
            'use_position': True,
            'method': result.get('method', 'position_bias_bearing_task'),
            'mean_error_deg': float(validation.get('mean_error_deg', 0.0)),
            'std_error_deg': float(validation.get('std_error_deg', 0.0)),
            'max_error_deg': float(validation.get('max_error_deg', 0.0)),
            'azimuth_offset_median': self.calibration_result['azimuth_offset'],
            'pitch_offset_median': self.calibration_result['pitch_offset'],
            'geometry_condition': geometry_condition,
            'position_bias_type': 'task_driven_equivalent_offset',
        }
        return True

    def _within_position_bounds(self, dx, dy, dz):
        return (
            abs(float(dx)) <= CALIBRATION_MAX_OFFSET_X_M
            and abs(float(dy)) <= CALIBRATION_MAX_OFFSET_Y_M
            and abs(float(dz)) <= CALIBRATION_MAX_OFFSET_Z_M
        )

    def to_cal_offset_measurements(self):
        """转换在线配对数据为 tools/cal_offset.py 使用的测量格式。"""
        measurements = []
        for r, o in zip(self.radar_measurements, self.optical_measurements):
            try:
                radar_range = float(r.get('range', 0))
                if radar_range < CALIBRATION_MIN_RANGE:
                    continue
                measurements.append({
                    'radar_az': float(r['azimuth']),
                    'radar_pitch': float(r['pitch']),
                    'radar_range': radar_range,
                    'optical_az': float(o['azimuth']),
                    'optical_pitch': float(o['pitch']),
                })
            except (KeyError, TypeError, ValueError):
                continue
        return measurements

    def get_source_measurements(self):
        """生成本次校准使用的原始配对数据快照。"""
        samples = []
        for r, o in zip(self.radar_measurements, self.optical_measurements):
            try:
                radar_range = float(r.get('range', 0))
                if radar_range < CALIBRATION_MIN_RANGE:
                    continue

                radar_ts = float(r['timestamp']) if r.get('timestamp') is not None else None
                optical_ts = float(o['timestamp']) if o.get('timestamp') is not None else None
                time_diff = None
                if radar_ts is not None and optical_ts is not None:
                    time_diff = abs(radar_ts - optical_ts)

                samples.append({
                    'index': len(samples) + 1,
                    'time_diff_sec': time_diff,
                    'radar': {
                        'timestamp': radar_ts,
                        'device_timestamp': r.get('device_timestamp'),
                        'track_id': r.get('track_id', self.current_target_id),
                        'is_tas': r.get('is_tas'),
                        'track_mode': r.get('track_mode'),
                        'azimuth': float(r['azimuth']),
                        'pitch': float(r['pitch']),
                        'range': radar_range,
                    },
                    'optical': {
                        'timestamp': optical_ts,
                        'device_timestamp': o.get('device_timestamp'),
                        'azimuth': float(o['azimuth']),
                        'pitch': float(o['pitch']),
                        'range': float(o.get('range', 0) or 0),
                        'status': o.get('status'),
                    },
                })
            except (KeyError, TypeError, ValueError):
                continue

        return {
            'target_id': self.current_target_id,
            'sample_count': len(samples),
            'min_radar_range_m': float(CALIBRATION_MIN_RANGE),
            'pair_time_window_sec': float(CALIBRATION_PAIR_TIME_WINDOW),
            'samples': samples,
        }

    def _with_source_measurements(self, result):
        data = dict(result)
        source = self.get_source_measurements()
        if source['sample_count'] == 0 and isinstance(result.get('source_measurements'), dict):
            source = result['source_measurements']
        data['source_measurements'] = source
        return data

    def _get_segment_measurements(self, range_min_m, range_max_m):
        measurements = []
        for r, o in zip(self.radar_measurements, self.optical_measurements):
            try:
                radar_range = float(r.get('range', 0))
                if radar_range < CALIBRATION_MIN_RANGE:
                    continue
                if radar_range < range_min_m or radar_range > range_max_m:
                    continue
                measurements.append({
                    'radar_az': float(r['azimuth']),
                    'radar_pitch': float(r['pitch']),
                    'radar_range': radar_range,
                    'optical_az': float(o['azimuth']),
                    'optical_pitch': float(o['pitch']),
                })
            except (KeyError, TypeError, ValueError):
                continue
        return measurements

    def _build_segment_source_measurements(self, range_min_m, range_max_m):
        source = self.get_source_measurements()
        samples = []
        for item in source.get('samples', []):
            radar = item.get('radar', {})
            radar_range = float(radar.get('range', 0) or 0)
            if range_min_m <= radar_range <= range_max_m:
                samples.append(item)
        return {
            'target_id': source.get('target_id'),
            'sample_count': len(samples),
            'min_radar_range_m': float(range_min_m),
            'max_radar_range_m': float(range_max_m),
            'pair_time_window_sec': source.get('pair_time_window_sec'),
            'samples': samples,
        }

    def calculate_segmented_6dof_params_v2(self):
        self.segmented_6dof_params = []
        stable_ranges = self.analyze_radar_stability().get('stable_ranges', [])
        if not stable_ranges:
            print("[calibration] no stable range detected, skip segmented parameter calculation")
            return False

        try:
            from tools.cal_offset import calculate_calibration_from_measurements
        except ImportError as exc:
            print(f"[calibration] cannot import tools.cal_offset: {exc}")
            return False

        success_count = 0
        for index, stable_range in enumerate(stable_ranges, start=1):
            range_min_m = float(stable_range['range_min_m'])
            range_max_m = float(stable_range['range_max_m'])
            range_label = f"{range_min_m:.0f}-{range_max_m:.0f}m"
            measurements = self._get_segment_measurements(range_min_m, range_max_m)
            if len(measurements) < STABILITY_MIN_SAMPLES_PER_BIN:
                print(f"[calibration] stable range {range_label} has too few samples, skip")
                continue

            is_low_pitch = self._is_low_pitch_segment(stable_range)
            if is_low_pitch:
                result = self._calculate_low_pitch_segment_params(measurements, range_min_m, range_max_m)
            else:
                result = calculate_calibration_from_measurements(
                    measurements,
                    min_samples=STABILITY_MIN_SAMPLES_PER_BIN,
                )
                result['segment_type'] = 'normal'
                result['vertical_weak'] = False
                result['segment_solver'] = result.get('method', 'segmented_6dof')

            if not result.get('success'):
                print(f"[calibration] stable range {range_label} parameter calculation failed: {result.get('reason', 'unknown reason')}")
                continue

            offset = result['offset']
            validation = result.get('validation', {})
            component_errors = result.get('component_errors') or self._compute_component_errors(offset, measurements)
            segment_params = _empty_segmented_6dof_entry()
            segment_params.update({
                'dx': float(offset[0]),
                'dy': float(offset[1]),
                'dz': float(offset[2]),
                'az_rotation': 0.0,
                'pitch_rotation': 0.0,
                'mean_error_deg': float(validation.get('mean_error_deg', 0.0)),
                'std_error_deg': float(validation.get('std_error_deg', 0.0)),
                'max_error_deg': float(validation.get('max_error_deg', 0.0)),
                'timestamp': time.time(),
                'sample_count': int(result.get('sample_count', len(measurements))),
                'method': 'segmented_position_bias_bearing_task',
                'range_min_m': range_min_m,
                'range_max_m': range_max_m,
                'range_label': range_label,
                'segment_type': result.get('segment_type', 'normal'),
                'segment_solver': 'segmented_position_bias_bearing_task',
                'vertical_weak': bool(result.get('vertical_weak', False)),
                'mean_az_error_deg': float(component_errors.get('mean_az_error_deg', 0.0)),
                'max_az_error_deg': float(component_errors.get('max_az_error_deg', 0.0)),
                'mean_pitch_error_deg': float(component_errors.get('mean_pitch_error_deg', 0.0)),
                'max_pitch_error_deg': float(component_errors.get('max_pitch_error_deg', 0.0)),
                'source_measurements': self._build_segment_source_measurements(range_min_m, range_max_m),
                'geometry_condition': float(result.get('geometry_condition', 0.0) or 0.0),
            })
            usable, usable_reason = self._evaluate_segment_quality(segment_params)
            segment_params['usable'] = bool(usable)
            segment_params['usable_reason'] = usable_reason
            self.segmented_6dof_params.append(segment_params)
            success_count += 1
            print(
                f"[calibration] stable range {index} {range_label} position-bias ready: "
                f"dx={segment_params['dx']:.2f}m, dy={segment_params['dy']:.2f}m, dz={segment_params['dz']:.2f}m, "
                f"mean error={segment_params['mean_error_deg']:.3f}deg, usable={segment_params['usable']}"
            )

        self.segmented_6dof_params.sort(key=lambda item: item.get('range_min_m', 0.0))
        return success_count > 0

    def calculate_segmented_6dof_params(self):
        self.segmented_6dof_params = []
        stable_ranges = self.analyze_radar_stability().get('stable_ranges', [])
        if not stable_ranges:
            print("[校准] 未识别到稳定距离段，跳过分段参数计算")
            return False

        try:
            from tools.cal_offset import calculate_calibration_from_measurements
        except ImportError as exc:
            print(f"[校准] 无法导入 tools.cal_offset: {exc}")
            return False

        success_count = 0
        for index, stable_range in enumerate(stable_ranges, start=1):
            range_min_m = float(stable_range['range_min_m'])
            range_max_m = float(stable_range['range_max_m'])
            range_label = f"{range_min_m:.0f}-{range_max_m:.0f}m"
            measurements = self._get_segment_measurements(range_min_m, range_max_m)
            if len(measurements) < STABILITY_MIN_SAMPLES_PER_BIN:
                print(f"[校准] 稳定段 {range_label} 样本不足，跳过")
                continue

            result = calculate_calibration_from_measurements(
                measurements,
                min_samples=STABILITY_MIN_SAMPLES_PER_BIN,
            )
            if not result.get('success'):
                print(f"[校准] 稳定段 {range_label} 参数计算失败: {result.get('reason', '未知原因')}")
                continue

            offset = result['offset']
            validation = result.get('validation', {})
            segment_params = _empty_segmented_6dof_entry()
            segment_params.update({
                'dx': float(offset[0]),
                'dy': float(offset[1]),
                'dz': float(offset[2]),
                'az_rotation': 0.0,
                'pitch_rotation': 0.0,
                'mean_error_deg': float(validation.get('mean_error_deg', 0.0)),
                'std_error_deg': float(validation.get('std_error_deg', 0.0)),
                'max_error_deg': float(validation.get('max_error_deg', 0.0)),
                'timestamp': time.time(),
                'sample_count': int(result.get('sample_count', len(measurements))),
                'method': result.get('method', 'segmented_6dof'),
                'range_min_m': range_min_m,
                'range_max_m': range_max_m,
                'range_label': range_label,
                'source_measurements': self._build_segment_source_measurements(range_min_m, range_max_m),
            })
            self.segmented_6dof_params.append(segment_params)
            success_count += 1
            print(
                f"[校准] 稳定段 {index} {range_label} 参数完成: "
                f"dx={segment_params['dx']:.2f}m, dy={segment_params['dy']:.2f}m, dz={segment_params['dz']:.2f}m, "
                f"误差均值={segment_params['mean_error_deg']:.3f}°"
            )

        self.segmented_6dof_params.sort(key=lambda item: item.get('range_min_m', 0.0))
        return success_count > 0

    def has_6dof_params(self):
        if any(item.get('usable', False) for item in self.segmented_6dof_params):
            return True
        return (
            self.far_params.get('sample_count', 0) > 0
            and self.near_params.get('sample_count', 0) > 0
            and (
                self.far_params.get('mean_error_deg', 0) > 0
                or self.near_params.get('mean_error_deg', 0) > 0
            )
        )

    def apply_calibration(self, radar_azimuth, radar_pitch, radar_range=None):
        """应用校准参数，将雷达角度转换为光电应该转到的角度。"""
        if self.position_offset.get('use_position', False) and radar_range is not None and radar_range > 0:
            if self.has_6dof_params():
                return self.apply_6dof_calibration(radar_azimuth, radar_pitch, radar_range)
            return self.apply_position_offset(radar_azimuth, radar_pitch, radar_range)

        return self.apply_angle_offset(radar_azimuth, radar_pitch)

    def apply_angle_offset(self, radar_azimuth, radar_pitch):
        calibrated_az = (radar_azimuth + self.calibration_result['azimuth_offset']) % 360
        calibrated_pitch = radar_pitch + self.calibration_result['pitch_offset']
        return calibrated_az, calibrated_pitch, None

    def apply_position_offset(self, radar_azimuth, radar_pitch, radar_range):
        if self.position_offset['sample_count'] == 0:
            return self.apply_angle_offset(radar_azimuth, radar_pitch)

        az_rad = np.radians(radar_azimuth)
        pitch_rad = np.radians(radar_pitch)
        target_x = radar_range * np.cos(pitch_rad) * np.sin(az_rad)
        target_y = radar_range * np.cos(pitch_rad) * np.cos(az_rad)
        target_z = radar_range * np.sin(pitch_rad)

        rel_x = target_x - self.position_offset['dx']
        rel_y = target_y - self.position_offset['dy']
        rel_z = target_z - self.position_offset['dz']

        opt_range = np.sqrt(rel_x**2 + rel_y**2 + rel_z**2)
        opt_azimuth = np.degrees(np.arctan2(rel_x, rel_y))
        if opt_azimuth < 0:
            opt_azimuth += 360
        opt_pitch = np.degrees(np.arcsin(rel_z / opt_range)) if opt_range > 0 else 0

        return opt_azimuth, opt_pitch, max(0.0, radar_range - FAKE_DIS)

    def apply_6dof_calibration(self, radar_azimuth, radar_pitch, radar_range):
        if radar_range is None or radar_range <= 0:
            return self.apply_angle_offset(radar_azimuth, radar_pitch)

        for params in self.segmented_6dof_params:
            if params.get('sample_count', 0) <= 0:
                continue
            if not params.get('usable', False):
                continue
            range_min_m = float(params.get('range_min_m', 0.0))
            range_max_m = float(params.get('range_max_m', 0.0))
            if range_min_m <= radar_range <= range_max_m:
                return self._apply_6dof_with_params(radar_azimuth, radar_pitch, radar_range, params)

        params = self.near_params if radar_range < self.RANGE_THRESHOLD else self.far_params
        if params.get('sample_count', 0) <= 0:
            return self.apply_position_offset(radar_azimuth, radar_pitch, radar_range)

        return self._apply_6dof_with_params(radar_azimuth, radar_pitch, radar_range, params)

    def _apply_6dof_with_params(self, radar_azimuth, radar_pitch, radar_range, params):
        if params.get('sample_count', 0) <= 0:
            return self.apply_position_offset(radar_azimuth, radar_pitch, radar_range)

        az_rad = np.radians(radar_azimuth)
        pitch_rad = np.radians(radar_pitch)
        x = radar_range * np.cos(pitch_rad) * np.sin(az_rad)
        y = radar_range * np.cos(pitch_rad) * np.cos(az_rad)
        z = radar_range * np.sin(pitch_rad)

        az_rot = np.radians(params.get('az_rotation', 0.0))
        pitch_rot = np.radians(params.get('pitch_rotation', 0.0))

        x1 = x * np.cos(az_rot) - y * np.sin(az_rot)
        y1 = x * np.sin(az_rot) + y * np.cos(az_rot)
        z1 = z

        x2 = x1
        y2 = y1 * np.cos(pitch_rot) - z1 * np.sin(pitch_rot)
        z2 = y1 * np.sin(pitch_rot) + z1 * np.cos(pitch_rot)

        rel_x = x2 - params.get('dx', 0.0)
        rel_y = y2 - params.get('dy', 0.0)
        rel_z = z2 - params.get('dz', 0.0)

        opt_range = np.sqrt(rel_x**2 + rel_y**2 + rel_z**2)
        opt_az = np.degrees(np.arctan2(rel_x, rel_y))
        if opt_az < 0:
            opt_az += 360
        opt_pitch = np.degrees(np.arcsin(rel_z / opt_range)) if opt_range > 0 else 0

        return opt_az, opt_pitch, max(0.0, radar_range - FAKE_DIS)

    def load_calibration(self):
        file_path = os.path.join(self.data_dir, 'calibration_params.json')
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                loaded = json.load(f)
                self.calibration_result.update(loaded)
            print(
                f"[校准] 已加载角度偏移: 方位={self.calibration_result['azimuth_offset']:.2f}°, "
                f"俯仰={self.calibration_result['pitch_offset']:.2f}°"
            )
            return True
        return False

    def save_calibration(self):
        file_path = os.path.join(self.data_dir, 'calibration_params.json')
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(self._with_source_measurements(self.calibration_result), f, indent=2, ensure_ascii=False)
        print(f"[校准] 角度偏移参数已保存: {file_path}")

    def load_position_offset(self):
        file_path = os.path.join(self.data_dir, 'position_offset.json')
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                loaded = json.load(f)
                self.position_offset.update(loaded)
            print(
                f"[校准] 已加载位置偏移: DX={self.position_offset['dx']:.2f}m, "
                f"DY={self.position_offset['dy']:.2f}m, dz={self.position_offset['dz']:.2f}m"
            )
            return True
        return False

    def save_position_offset(self):
        file_path = os.path.join(self.data_dir, 'position_offset.json')
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(self._with_source_measurements(self.position_offset), f, indent=2, ensure_ascii=False)
        print(f"[校准] 位置偏移已保存: {file_path}")

    def load_6dof_params(self):
        file_path = os.path.join(self.data_dir, '6dof_params.json')
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.far_params.update(data.get('far', {}))
                self.near_params.update(data.get('near', {}))
                self.segmented_6dof_params = []
                for item in data.get('segments', []):
                    segment = _empty_segmented_6dof_entry()
                    segment.update(item)
                    self.segmented_6dof_params.append(segment)
            print("[校准] 已加载6DoF参数")
            return True
        return False

    def save_6dof_params(self):
        file_path = os.path.join(self.data_dir, '6dof_params.json')
        usable_segments = [item for item in self.segmented_6dof_params if item.get('usable', False)]
        preferred_segment = None
        if usable_segments:
            preferred_segment = min(
                usable_segments,
                key=lambda item: (float(item.get('mean_error_deg', 9999.0)), -int(item.get('sample_count', 0)))
            ).get('range_label')
        data = {
            'preferred_segment': preferred_segment,
            'usable_segments': [item.get('range_label') for item in usable_segments],
            'usable_segment_ranges': [
                {
                    'range_label': item.get('range_label'),
                    'range_min_m': item.get('range_min_m'),
                    'range_max_m': item.get('range_max_m'),
                    'mean_error_deg': item.get('mean_error_deg'),
                    'max_error_deg': item.get('max_error_deg'),
                    'segment_type': item.get('segment_type'),
                    'vertical_weak': item.get('vertical_weak', False),
                }
                for item in usable_segments
            ],
            'far': self.far_params,
            'near': self.near_params,
            'segments': self.segmented_6dof_params,
            'range_threshold': self.RANGE_THRESHOLD,
            'timestamp': time.time(),
            'source_measurements': self.get_source_measurements(),
        }
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f"[校准] 6DoF参数已保存: {file_path}")

    def _delete_calibration_file(self, filename):
        file_path = os.path.join(self.data_dir, filename)
        if os.path.exists(file_path):
            os.remove(file_path)
            print(f"[calibration] deleted parameter file: {file_path}")

    def get_status(self):
        return {
            'mode': self.calibration_mode,
            'target_id': self.current_target_id,
            'target_history': list(self.calibration_target_history),
            'radar_samples': len(self.radar_measurements),
            'optical_samples': len(self.optical_measurements),
            'radar_stability_samples': len(self.radar_stability_samples),
            'radar_stable_ranges': self.analyze_radar_stability().get('stable_ranges', []),
            'has_calibration': self.calibration_result['sample_count'] > 0,
            'has_position': self.position_offset['sample_count'] > 0,
            'use_position': self.position_offset.get('use_position', False),
            'has_6dof': self.has_6dof_params(),
        }

    def clear_calibration(self):
        """清除校准参数和本次采集样本。"""
        self.radar_measurements = []
        self.optical_measurements = []
        self.current_target_id = None
        self.calibration_mode = False
        self.calibration_target_history = []
        self.radar_stability_samples = []
        self.radar_stability_report = self._empty_stability_report()
        self.radar_buffer.clear()
        self.optical_buffer.clear()
        self.radar_smoother.reset()
        self.optical_smoother.reset()
        self.calibration_result = {
            'azimuth_offset': 0.0,
            'pitch_offset': 0.0,
            'azimuth_scale': 1.0,
            'pitch_scale': 1.0,
            'timestamp': 0,
            'sample_count': 0,
        }
        self.position_offset = {
            'dx': 0.0,
            'dy': 0.0,
            'dz': 0.0,
            'timestamp': 0,
            'sample_count': 0,
            'use_position': False,
        }
        self.far_params = _empty_6dof_params('far_6dof')
        self.near_params = _empty_6dof_params('near_6dof')
        self.segmented_6dof_params = []
        self._delete_calibration_file('calibration_params.json')
        self._delete_calibration_file('position_offset.json')
        self._delete_calibration_file('6dof_params.json')
        print("[校准] 已清除所有校准参数")


calibrator = RadarOpticalCalibrator()
