...
class OpticalTracker:
    def __init__(self, device_ip="192.168.0.4", local_ip="192.168.0.9", port=9966, device_port=None):
        self.device_ip = device_ip
        self.local_ip = local_ip
        self.port = port
        self.device_port = device_port if device_port is not None else port
        self.sock = None
        self.seq = 1
        ...
        self.latest_angle_host_time = None
        self.latest_status_host_time = None
        self.latest_target_host_time = None
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"[连接] 目标设备 {self.device_ip}:{self.device_port}")
            return True
    ...
    def _print_packet_info(self, data, addr):
        ...
        if cmd == 0x08 and len(data) >= 20:
            ...
            if len(data) >= 40:
                ...
                with self.lock:
                    ...
                    self.latest_status_host_time = time.time()
        ...
        if cmd == 0x0B:
            ...
            with self.lock:
                ...
                self.latest_target_host_time = time.time()
    ...
    def _parse_target_info(self, data):
        ...
        try:
            packet_timestamp = struct.unpack('<I', data[24:28])[0]
