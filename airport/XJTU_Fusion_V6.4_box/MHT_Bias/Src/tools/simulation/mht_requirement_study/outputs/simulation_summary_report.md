...
- 通过组合数：`1`
- 通过比例：`100.00%`
- 推荐保守配置：`N >= 3`，`Pd >= 0.70`，`sigma <= 24.0m`
- 在当前通过组合中，最高验证通过的输入虚警率：`0.40`

## 4. 稳定通过区间总结

- 传感器数量区间：`3 ~ 3`
- 单传感器检测率区间：`0.7 ~ 0.7`
- 单传感器位置误差标准差区间：`24.0 ~ 24.0 m`
- 单传感器输入虚警率区间：`0.4 ~ 0.4`
...
- 输入：`N=3`，`Pd=0.70`，`sigma=24.0m`，`Pfa_in=0.40`
- 输出：`Pd_out=0.958`，`RMSE=14.85m`，`RMSE_Z=8.10m`，`Pfa_out=0.012`
