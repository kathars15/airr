...
# 越大，漏检后航迹更容易被判定为结束。
NUM = 3
MHT_RESOLVED_TIME_WINDOW_SEC = 2.0 * NUM
# 航迹确认时间窗，单位秒。
# 在这个时间窗内满足一定检测次数后，航迹才会进入确认状态。

MHT_RESOLVED_MIN_DETECT = 4
# 航迹确认所需的最小检测次数。
# 当前设为 1，表示更偏向快速确认；如果改大，会更保守。

MHT_MAX_DETECT_TIME_SEC = 2
...
class RunMetrics:
    ...
    output_tracks: int
    false_tracks: int
    false_track_events: Optional[int] = None
    false_track_unique_est: Optional[int] = None
    output_track_events: Optional[int] = None
    false_origin_track_events: Optional[int] = None
    false_origin_track_unique_est: Optional[int] = None
    false_track_longest_streak: Optional[int] = None
...
class TraceFrame:
    ...
    measurement_positions: List[List[float]]
...
def compute_match_gate_m(sensor_pos_std_m: float) -> float:
    ...
    return MATCH_GATE_ROOT_SCALE * sigma


def compute_axis_std_from_3d_rmse(sensor_pos_std_m: float) -> float:
    sigma_3d = max(float(sensor_pos_std_m), 1e-6)
    return sigma_3d / math.sqrt(3.0)


def compute_expected_false_count_per_event(config: ScenarioConfig) -> float:
    pfa = min(max(float(config.sensor_fa_prob), 0.0), 0.999999)
    if pfa <= 0.0:
        return 1e-6
    expected_truth = max(TARGET_COUNT * float(config.sensor_pd), 0.0)
    expected_false = (pfa / max(1.0 - pfa, 1e-6)) * expected_truth
    return max(expected_false, 1e-6)


def quantize_false_count(target_false_count: float, rng: np.random.Generator) -> int:
    if target_false_count <= 0.0:
        return 0
    lower = int(math.floor(target_false_count))
    frac = target_false_count - lower
    return lower + (1 if rng.random() < frac else 0)


def build_sensor_event_schedule(sensor_count: int) -> List[float]:
    count = max(int(sensor_count), 1)
    if count == 1:
        return [0.0]
    step = SCAN_PERIOD_SEC / float(count)
    return [idx * step for idx in range(count)]


def should_evaluate_outputs(sensor_idx: int, sensor_count: int) -> bool:
    """Evaluate once per completed asynchronous sensor round.

    Each sensor still revisits at `SCAN_PERIOD_SEC`; only the arrival phases are
    staggered. For output metrics we therefore sample after the last sensor in a
    round has reported, instead of after every asynchronous sub-event.
    """
    return int(sensor_idx) == max(int(sensor_count) - 1, 0)
...
def generate_measurements(
    ...
    axis_std = compute_axis_std_from_3d_rmse(config.sensor_pos_std_m)
    truth_count = 0
    for target in targets:
        if rng.random() <= config.sensor_pd:
            noise = rng.normal(0.0, axis_std, size=3)
    ...
    if truth_count > 0:
        false_target = (config.sensor_fa_prob / max(1.0 - config.sensor_fa_prob, 1e-6)) * truth_count
        false_count = quantize_false_count(false_target, rng)
...
def build_sensor_config(config: ScenarioConfig, sensor_idx: int = 0) -> Dict[str, object]:
    # 把“单传感器虚警概率”映射成 POMHT 打分侧需要的期望虚警数量。
    # 这里保留一个很小的下限，避免出现 log(0)。
    axis_std = compute_axis_std_from_3d_rmse(config.sensor_pos_std_m)
    fa_num = compute_expected_false_count_per_event(config)
    sensor_config: Dict[str, object] = {
        "Name": f"Sim_XYZ_{int(sensor_idx)}",
        "Position": np.array([[0.0, 0.0, 0.0]]).T,
        "Meas_Type": "Position",
        "R": np.power(np.diag([axis_std] * 3), 2),
    ...
    sensor_config["lambda_death"] = -math.log(1.0 - 0.01)
...
def get_output_positions(tracker: POMHT_Bias) -> List[np.ndarray]:
    ...
    for node in deepcopy(tracker.Output_Nodes[-1]).values():
        if getattr(node, "hyp_type", "") == "death":
            continue
...
def get_output_nodes(tracker: POMHT_Bias) -> List[object]:
    if not hasattr(tracker, "Output_Nodes") or not tracker.Output_Nodes:
        return []
    nodes: List[object] = []
    for node in deepcopy(tracker.Output_Nodes[-1]).values():
...
def run_single_trial(
    ...
    output_tracks = 0
    false_tracks = 0
    false_track_events = 0
    output_track_events = 0
    false_track_labels: set[int] = set()
    false_origin_track_events = 0
    false_origin_track_labels: set[int] = set()
    false_track_streaks: Dict[int, int] = {}
    false_track_longest_streak = 0
    errors: List[float] = []
    z_errors: List[float] = []
    trace_frames: List[TraceFrame] = []

    sensor_next_times = build_sensor_event_schedule(config.sensor_count)
    last_timestamp = 0.0
    event_idx = 0
    while True:
        sensor_idx = min(range(len(sensor_next_times)), key=lambda idx: (sensor_next_times[idx], idx))
        timestamp = float(sensor_next_times[sensor_idx])
        if timestamp > SIM_DURATION_SEC + 1e-9:
            break

        dt = max(timestamp - last_timestamp, 0.0)
        if event_idx > 0 and dt > 0.0:
            targets = propagate_targets(targets, dt, rng, event_idx)
        last_timestamp = timestamp

        obs_k, infos = generate_measurements(targets, config, rng, sensor_idx)
        sensor_config = build_sensor_config(config, sensor_idx=sensor_idx)
        if tracker is None:
        ...
        sensor_next_times[sensor_idx] += SCAN_PERIOD_SEC
        event_idx += 1

        if timestamp < WARMUP_SEC:
            continue
        if not should_evaluate_outputs(sensor_idx, config.sensor_count):
            continue

        output_nodes = get_output_nodes(tracker)
        outputs = [
            np.asarray(node.x_k_k[:3, :], dtype=float).reshape(3)
        ...
        output_track_events += len(output_nodes)
        errors.extend(frame_errors)
        if output_nodes:
            cost = np.zeros((len(output_nodes), len(targets)), dtype=float)
            for i, node in enumerate(output_nodes):
                out_pos = np.asarray(node.x_k_k[:3, :], dtype=float).reshape(3)
                for j, target in enumerate(targets):
                    cost[i, j] = float(np.linalg.norm(out_pos - target.pos))
            row_ind, col_ind = linear_sum_assignment(cost) if len(targets) > 0 else (np.array([], dtype=int), np.array([], dtype=int))
            matched_rows = set()
            for row, col in zip(row_ind, col_ind):
                if float(cost[row, col]) <= match_gate_m:
                    matched_rows.add(int(row))
            for row_idx, node in enumerate(output_nodes):
                label = getattr(node, "label", None)
                if row_idx not in matched_rows:
                    false_track_events += 1
                    if label is not None:
                        false_track_labels.add(int(label))
                        current_streak = false_track_streaks.get(int(label), 0) + 1
                        false_track_streaks[int(label)] = current_streak
                        false_track_longest_streak = max(false_track_longest_streak, current_streak)
                    origin_kind = getattr(node, "debug_info", {}).get("origin_kind", None)
                    if origin_kind == "false_alarm":
                        false_origin_track_events += 1
                        if label is not None:
                            false_origin_track_labels.add(int(label))
                elif label is not None:
                    false_track_streaks[int(label)] = 0
            ...
            trace_frames.append(
                TraceFrame(
                    ...
                    measurement_positions=[
                        np.asarray(obs, dtype=float).reshape(3).tolist()
                        for obs in obs_k
                    ],
                    measurement_kinds=[
                        str(item.get("kind", "unknown")) if isinstance(item, dict) else "unknown"
                        for item in infos
                    ],
                    output_positions=[np.asarray(pos, dtype=float).reshape(3).tolist() for pos in outputs],
                    output_labels=[
                        int(getattr(node, "label"))
                        for node in output_nodes
                    ],
    ...
    return RunMetrics(
        ...
        false_track_events=false_track_events,
        false_track_unique_est=len(false_track_labels),
        output_track_events=output_track_events,
        false_origin_track_events=false_origin_track_events,
        false_origin_track_unique_est=len(false_origin_track_labels),
        false_track_longest_streak=false_track_longest_streak,
...
def summarize_trials(config: ScenarioConfig, trial_metrics: Sequence[RunMetrics]) -> Dict[str, object]:
    ...
    trial_detection_rate_mean = mean_attr("detection_rate", none_as=0.0)
    trial_false_alarm_rate_mean = mean_attr("false_alarm_rate", none_as=0.0)
    detection_rate = float(trial_detection_rate_mean) if trial_detection_rate_mean is not None else 0.0
    false_alarm_rate = float(trial_false_alarm_rate_mean) if trial_false_alarm_rate_mean is not None else 0.0
    ...
    return {
        ...
        "trial_detection_rate_mean": trial_detection_rate_mean,
        "trial_false_alarm_rate_mean": trial_false_alarm_rate_mean,
        "trial_rmse_m_mean": mean_attr("rmse_m"),
        "trial_rmse_z_m_mean": mean_attr("rmse_z_m"),
        "aggregate_detection_rate": (total_matched / total_truth if total_truth > 0 else 0.0),
        "aggregate_false_alarm_rate": (total_false / total_outputs if total_outputs > 0 else 0.0),
    }
...
def _measurement_series(
    ...
    up: List[float] = []
    for frame in trace_frames:
        for pos, pos_kind in zip(frame.measurement_positions, frame.measurement_kinds):
            if kind is not None and pos_kind != kind:
                continue
            times.append(float(frame.timestamp))
            east.append(float(pos[0]))
            north.append(float(pos[1]))
            up.append(float(pos[2]))
    return times, east, north, up


def _filtered_false_alarm_series(
    trace_frames: Sequence[TraceFrame],
    near_radius_m: float,
) -> Tuple[List[float], List[float], List[float], List[float]]:
    return _measurement_series(trace_frames, kind="false_alarm")


def _output_track_series_by_label(
    trace_frames: Sequence[TraceFrame],
) -> Dict[int, Dict[str, List[float]]]:
    tracks: Dict[int, Dict[str, List[float]]] = {}
    for frame in trace_frames:
        for label, pos in zip(frame.output_labels, frame.output_positions):
            series = tracks.setdefault(
                int(label),
                {"time": [], "east": [], "north": [], "up": []},
            )
            series["time"].append(float(frame.timestamp))
            series["east"].append(float(pos[0]))
            series["north"].append(float(pos[1]))
            series["up"].append(float(pos[2]))
    return tracks


def _track_color(label: int, cmap_name: str = "tab20") -> Tuple[float, float, float, float]:
    cmap = plt.get_cmap(cmap_name)
    return cmap(int(label) % max(getattr(cmap, "N", 20), 1))
...
def plot_trace_comparison_3d(
    ...
    truth_meas_t, truth_meas_e, truth_meas_n, truth_meas_u = _measurement_series(trace_frames, kind="truth")
    output_tracks = _output_track_series_by_label(trace_frames)
    x_all: List[float] = []
    y_all: List[float] = []
    z_all: List[float] = []
    if truth_meas_e:
        ax.scatter(truth_meas_e, truth_meas_n, truth_meas_u, s=10, c="tab:cyan", alpha=0.22, label="Measurements")
        x_all.extend(truth_meas_e)
        y_all.extend(truth_meas_n)
        z_all.extend(truth_meas_u)
    for label, series in sorted(output_tracks.items()):
        color = _track_color(label)
        ax.plot(
            series["east"],
            series["north"],
            series["up"],
        ...
        x_all.extend(series["east"])
        y_all.extend(series["north"])
        z_all.extend(series["up"])
    ...
    ax.set_title(
        "Measurement cloud and tracked trajectories\n"
        f"N={config.sensor_count}, Pd={config.sensor_pd:.2f}, sigma3d={config.sensor_pos_std_m:.1f}m, "
...
def plot_trace_comparison_timeseries(
    ...
    output_tracks = _output_track_series_by_label(trace_frames)
    truth_meas_t, truth_meas_e, truth_meas_n, truth_meas_u = _measurement_series(trace_frames, kind="truth")
    meas_dims = [truth_meas_e, truth_meas_n, truth_meas_u]
    for dim_idx, ax in enumerate(axes):
        if truth_meas_t:
            ax.scatter(truth_meas_t, meas_dims[dim_idx], s=9, c="tab:cyan", alpha=0.22, label="Measurements")
        for label, series in sorted(output_tracks.items()):
            color = _track_color(label)
            track_values = series[["east", "north", "up"][dim_idx]]
            ax.plot(
                series["time"],
                track_values,
                color=color,
    ...
    axes[0].set_title(
        "Measurements and tracked trajectories by time\n"
        f"N={config.sensor_count}, Pd={config.sensor_pd:.2f}, sigma3d={config.sensor_pos_std_m:.1f}m, "
...
def plot_sweep_metric(
    ...
    ax.set_title(
        ...
        f"Pd={fixed_config.sensor_pd:.2f}, sigma3d={fixed_config.sensor_pos_std_m:.1f}m, "
...
def run_study(args: argparse.Namespace) -> Tuple[List[Dict[str, object]], List[Dict[str, object]]]:
    ...
    for cfg_idx, config in enumerate(configs, start=1):
        ...
        print(
            ...
            f"sigma3d={config.sensor_pos_std_m:.1f}m, Pfa_in={config.sensor_fa_prob:.2f}, "
...
def build_settings_dict(mc_runs: int, seed: int) -> Dict[str, object]:
    return {
        ...
        "match_gate_rule": "MATCH_GATE_M = 0.90 * sigma_3d_rmse",
        ...
        "input_false_alarm_model": "single-sensor event false-alarm ratio FA/All; false alarms appear only when truth detections exist",
        "position_error_model": "3D distance RMSE converted to isotropic XYZ Gaussian noise",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sensor-pos-std", action="append", help="single-sensor 3D position distance RMSE (m)")
    parser.add_argument("--sensor-fa-prob", action="append", help="single-sensor batch false-alarm ratio FA/All")
