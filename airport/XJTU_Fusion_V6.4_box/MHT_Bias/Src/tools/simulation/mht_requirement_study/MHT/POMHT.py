...
class POMHT_Bias():
    ...
    def hypotheses_new_tracks(
        ...
        for obs_id, obs_ in enumerate(self.obs_k):
            ...
            leaf_node.detection_timestamps = [copy(self.Timestamps[-1])]
            info_data = {}
            if isinstance(last_info, (list, tuple)) and obs_id < len(last_info):
                info_item = last_info[obs_id]
            ...
            leaf_node.debug_info['sensor_name'] = self.sensor_config['Name']
            leaf_node.debug_info['timestamp'] = copy(self.Timestamps[-1])
            leaf_node.debug_info['origin_kind'] = info_data.get('kind', None) if info_data else None
    ...
    def Generate_hyp_from_Leaf_nodes(
        ...
        if node.hyp_type == 'death':
            ...
            # 非死亡节点，波门后，往下生成多个假设
                ...
                P_D_current = float(self.sensor_config['P_D'])

                for io, obs_ in enumerate(self.obs_k):
                    info_data = {}
                    if isinstance(last_info, (list, tuple)) and io < len(last_info):
                        info_item = last_info[io]
                        if isinstance(info_item, dict):
                            info_data = info_item
                    elif isinstance(last_info, dict):
                        info_data = last_info
                ...
                if delta_tmp <= self.max_detect_time:
                    ...
                    new_leaf_node.debug_info['dt'] = self.T
                    new_leaf_node.debug_info['timestamp'] = copy(self.Timestamps[-1])
                    new_leaf_node.debug_info['origin_kind'] = node.debug_info.get('origin_kind', None)
                    
                    new_leaf_node.debug_info['droneName'] = node.debug_info.get('droneName', None)
                    new_leaf_node.debug_info['direction'] = node.debug_info.get('direction', 0.0)
                    new_leaf_node.debug_info['pilotLongitude'] = node.debug_info.get('pilotLongitude', 0.0)
                    new_leaf_node.debug_info['pilotLatitude'] = node.debug_info.get('pilotLatitude', 0.0)
                    new_leaf_node.debug_info['signalPowerDidch1'] = node.debug_info.get('signalPowerDidch1', None)
                    new_leaf_node.debug_info['deviceCode'] = node.debug_info.get('deviceCode', 'RADAR')
                    new_leaf_node.debug_info['sn'] = node.debug_info.get('sn', None)
